...
const CourseOrgSetting = () => {
  ...
  const handleRemoveCourse = async (courseId) => {
    try {
      const { status } = await removeOrgCourse(courseId);
      if (status === 200) {
                ...
                return (
                  <div className="product-box" key={course.id}>
                    <div className="buttons">
                      <Link to={`/edit-org-course/${course.id}`}>
                        <i className="edit">
                          <GrEdit />
                        </i>
                      </Link>
...
export default CourseOrgSetting;
